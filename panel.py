...
fbx/, jsons/, pdf/, islem_gecmisi.json yalnızca okunur; yazılan tek dosya
data/panel_checklist.json'dur (atomik: temp + fsync + os.replace).
...
class PanelIstek(BaseHTTPRequestHandler):
    ...
    def log_request(self, code="-", size="-"):
        ...
        if str(code).startswith(("2", "3")) and yol in ("/api/durum", "/saglik", "/favicon.ico"):
            return
    ...
    def _pdf(self, yol, indirme_adi):
        ...
        if not gercek.startswith(os.path.realpath(PDF_DIR) + os.sep):
            return self._json(404, {"hata": "bulunamadı"})
        try:
            f = open(gercek, "rb")
        except OSError:
            return self._json(404, {"hata": "PDF bulunamadı"})
        with f:
            boyut = os.fstat(f.fileno()).st_size
            self.send_response(200)
            self.send_header("Content-Type", "application/pdf")
            self.send_header("Content-Length", str(boyut))
            self.send_header("Content-Disposition", f'inline; filename="{indirme_adi}"')
            self.send_header("Cache-Control", "no-store")
            self.end_headers()
            if self.command == "HEAD":
                return
